...
#pragma endregion
		...
		std::cout << "Wind Direction = " << windDirectionArr[f_windDirection.value] << "\n";
		...
		while (yn != 0 || yn != 1) {
			yn = yesNoCheck(temp);
			if (yn == 1) {
				return WeatherFrameResponse(WeatherFrame(f_temperature, f_feelsLikeTemperature, f_heatIndex, f_dewPoint,
